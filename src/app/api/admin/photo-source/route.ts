...
import { bucket, objectKeyFromUrl, r2 } from '@/lib/storage/r2';

/**
 * Serves a stored photo back to the admin through our own origin, so the border
 * trimmer can read its pixels off a canvas. Fetching it straight from the R2
 * public host would taint the canvas unless the bucket happens to send CORS
 * headers, and `getImageData` on a tainted canvas throws — a failure that would
 * depend on bucket configuration rather than on anything in this repo.
 *
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    console.error('Could not read photo source', key, cause);

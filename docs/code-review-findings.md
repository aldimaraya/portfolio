...
| ~~15~~ | ~~6.1~~ | **FIXED.** `once()` now takes a budget and rejects with the wait it was in — `Could not seek to 4.20s — gave up after 30s` — so a file the browser demuxes but cannot decode fails the form rather than parking it. Metadata gets 60s and each seek 30s, both far past any honest worst case on purpose: a timeout that fires on a working clip fails routinely on real uploads, where the hang needs a broken file, so the cheap error is the one to be generous about. The existing `finally` still releases the decoder and object URL on this path — the rejection surfaces at an `await` inside the `try`. | `lib/video/sprite.ts` |
...
| ~~33~~ | ~~6.2~~ | **FIXED** alongside 6.1, which wraps the same loop. `SpriteOptions.onProgress` fires at 0 once the frame count is known and again per captured frame; `VideoForm` renders it in the line that already said "Grabbing frames…", as a percentage plus a count, matching how upload progress is worded so a save reads as one process rather than two. Percentages come from `spriteProgress`, which is clamped and unit-tested; the seek loop around it is not, since firing `seeked` needs a decoder jsdom does not have. | `lib/video/sprite.ts`, `components/admin/VideoForm.tsx` |

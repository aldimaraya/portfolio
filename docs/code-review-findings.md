...
| ~~11~~ | ~~2.1~~ | **FIXED.** A part now gets four attempts with exponential backoff (300 ms doubling, capped at 8 s), and only for failures a second attempt could fix: a thrown `fetch` — the dropped connection this exists for — plus 408, 429 and 5xx. A 401/403 is a session that expired mid-upload and a 4xx below is a request we will keep building wrong, so both fail immediately rather than making the admin wait three backoffs for the same message. A missing ETag is not retried either: the bucket's CORS policy will hide the header identically every time. Each attempt **re-signs** the part URL rather than re-PUTting the one it holds — a presigned URL has a fixed lifetime and the attempt most worth retrying is the one late in a multi-hundred-MB upload, so the URL may be the thing that expired; one same-origin round-trip is nothing against re-sending 5 MiB, and it makes a 403 from R2 a real refusal. `abortQuietly` still runs when the attempts are exhausted. Depended on 2.4 to tell those cases apart. `shouldRetry` and `backoffDelay` are pure and pinned. | `lib/storage/upload-client.ts` |
| 12 | N12 | **Deleting a photo or clip silently breaks every published post that embeds it.** `listPostMedia` exists so a post can reuse library media, and `mediaSnippet` writes the R2 URL straight into the body — but nothing records that the post points at that row. `deletePhoto`/`deleteVideo` drop the object and revalidate `/stills` and `/motion`; the journal pages, which are prerendered, keep serving a cached page whose image is now a 404. There is no back-reference to warn from and no way to find the damage afterwards. Cheapest honest fix is a scan of `blogPost.markdownContent` for the URL before deleting, and a refusal — or at minimum a `revalidatePath('/journal', 'layout')` so the breakage is at least visible. **Note:** the ordering fix for 8.3 does not touch this — it changes *when* the object is dropped, not whether anything checks who points at it. | `app/admin/photos/actions.ts:129`, `app/admin/videos/actions.ts:135` |
| ~~13~~ | ~~8.3~~ | **FIXED.** Every media delete now writes the row first and drops the object after, best-effort. Neither store spans a transaction with the other, so the only choice is which half-done state to accept: an object nothing points at is invisible and costs pennies, where a row outliving its file puts a 404 on a public page until someone notices. The failing URLs are logged so the orphan can still be swept by hand. `deleteVideo` carried the same ordering and its comment cited `deletePhoto` as precedent, so both moved together. | `app/admin/photos/actions.ts`, `app/admin/videos/actions.ts` |
| ~~14~~ | ~~N2~~ | **FIXED** alongside 8.3, which is the same defect in the mirror direction. `saveVideo` now drops a replaced poster and sprite *after* the `$transaction` that stops pointing at them, best-effort and logged rather than failing an edit that already landed. The disagreement this finding names — `retouchPhoto` resolving the identical choice the other way twelve lines above — is gone: every media write in the admin now orders the row first. | `app/admin/videos/actions.ts` |
...
| ~~17~~ | ~~8.4~~ | **FIXED.** `tagConnections` upserts sequentially. Two tags of one name cannot collide inside a single call — `parseTagNames` dedupes — but two saves overlapping in flight can, and one admin still has two tabs. A handful of names per save makes the extra round-trips unmeasurable. | `lib/tags/index.ts` |
...
| ~~27~~ | ~~2.4~~ | **FIXED.** `postJson` reads the route's `{ error }` body and throws that, falling back to the status only when there is no JSON to read — a gateway's HTML 502 has no message to surface. Failures now carry the status on an `UploadHttpError`, which is what lets 2.1 tell a retryable failure from a permanent one, so this landed first. | `lib/storage/upload-client.ts` |
...
| ~~34~~ | ~~2.5~~ | **FIXED.** Progress is byte-weighted, so a short final part advances the bar by what it actually carries. Credit is applied only once a part is confirmed, which is what keeps a retried part from being counted twice or the bar from running backwards under 2.1. Accepted limit: `fetch` exposes no upload-progress events, so a part remains the finest granularity available and a single-part file still steps 0 → 100 — fixing that means moving the PUT to `XMLHttpRequest`, which is a larger change than this finding is worth. | `lib/storage/upload-client.ts` |
...
| ~~46~~ | ~~N11~~ | **FIXED.** `uploadFile` refuses a zero-byte blob by name — "… is empty (0 bytes) — nothing to upload" — before it creates a multipart upload, so the failure no longer arrives four requests later as "Invalid completion request", and no empty upload is left to abort. `splitIntoParts` is unchanged: it is correct on every boundary case including this one, and the missing check was the caller's. | `lib/storage/upload-client.ts` |
...
case; #8.3 and #N2 settled them on one rule — row first, object after,
best-effort — since only one of the two half-done states is visible to a visitor.

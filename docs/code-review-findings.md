...
| ~~12~~ | ~~N12~~ | **FIXED.** `deletePhoto` and `deleteVideo` now refuse rather than break the journal: both ask `postsEmbeddingMedia` which entries name the media before dropping anything, and return the titles. A clip is checked on all three of its objects, since the picker writes the poster into the Markdown title beside the video URL. Both spellings of a URL are searched — the raw one the row holds and the percent-escaped one `mediaSnippet` writes — and the narrowing is a `contains` in the query rather than reading every body into memory, so only the matching rows cross the wire; the same pure `postsEmbedding` then decides, so the rule that is tested is the rule that runs. Drafts count, and are marked as such: a draft is the copy most likely to be published *after* the media is gone, and excluding it would tie the guard's coverage to a flag unrelated to whether the reference exists. No force flag — the only fix is to edit those posts, so the message says which. `DeleteButton` now renders the refusal on its own line so a multi-entry sentence wraps instead of pushing the row off the page. **Deferred:** the real fix is a back-reference table, which is a migration this repo has no history for; until then the body text is the index. | `lib/post/media-usage.ts`, `app/admin/photos/actions.ts`, `app/admin/videos/actions.ts` |
...
| ~~16~~ | ~~N3~~ | **FIXED.** All nine actions now run their body inside `attempt` from `lib/actions/errors.ts`, so a throw past the validation boundary comes back as the same `{ error }` the forms already render — one wrapper rather than nine copy-pasted `try`/`catch` blocks, and in a plain module rather than an action file, since everything exported from `'use server'` becomes a callable endpoint. The real cause is logged with the action's name; the browser gets a plain sentence, with `P2025`, `P2002`, `P2003` and the connection codes mapped by name and everything else falling back to a generic line rather than a confident wrong guess. Nothing returned names a table, a constraint, or a query. `listPostMedia` has no room for an error, so it takes `attemptOr` and answers with the empty library it already returns to an unauthenticated caller. Callers needed no change — every one already had an `if (result.error)` branch. Covered by tests, including that no mapped message echoes the thrown one. | `lib/actions/errors.ts`, `app/admin/{photos,videos,posts}/actions.ts` |
